...
- **Запущенное приложение видно человеку за машиной.** `swift run Chelka` рисует панель на настоящем экране владельца, поверх его работы. Поэтому: запускать только когда это правда нужно для проверки, **гасить сразу после** (`pkill -f "4elka.*/Chelka"`), и никогда не оставлять запущенным к моменту отчёта. В отчёте писать, что приложение погашено.
