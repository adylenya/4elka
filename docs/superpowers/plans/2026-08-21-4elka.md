...
- Produces:
  ...
  public struct MonthGrid: Equatable {
      ...
      public static func make(year: Int, month: Int, today: Date, calendar: Calendar) -> MonthGrid
      public func shifted(by months: Int, today: Date, calendar: Calendar) -> MonthGrid
  }
  ```
...
@Test func august2026StartsOnSaturdayWithMondayFirstWeek() {
    let g = MonthGrid.make(year: 2026, month: 8, today: date(2026, 8, 21), calendar: mondayFirst)
...
@Test func todayIsNotMarkedInOtherMonths() {
    let g = MonthGrid.make(year: 2026, month: 9, today: date(2026, 8, 21), calendar: mondayFirst)
...
@Test func februaryInLeapYearHas29Days() {
    let g = MonthGrid.make(year: 2028, month: 2, today: date(2026, 8, 21), calendar: mondayFirst)
    #expect(g.weeks.flatMap { $0 }.compactMap { $0?.number }.last == 29)
}

@Test func februaryInCommonYearHas28Days() {
    let g = MonthGrid.make(year: 2026, month: 2, today: date(2026, 8, 21), calendar: mondayFirst)
    #expect(g.weeks.flatMap { $0 }.compactMap { $0?.number }.last == 28)
}

@Test func shiftingWrapsAcrossYearBoundary() {
    let december = MonthGrid.make(year: 2026, month: 12, today: date(2026, 8, 21), calendar: mondayFirst)
    let january = december.shifted(by: 1, today: date(2026, 8, 21), calendar: mondayFirst)
    #expect(january.year == 2027)
    #expect(january.month == 1)

    let previous = MonthGrid.make(year: 2026, month: 1, today: date(2026, 8, 21), calendar: mondayFirst)
        .shifted(by: -1, today: date(2026, 8, 21), calendar: mondayFirst)
...
@Test func weekdayTitlesFollowFirstWeekdaySetting() {
    ...
    let m = MonthGrid.make(year: 2026, month: 8, today: date(2026, 8, 21), calendar: mondayFirst)
    let s = MonthGrid.make(year: 2026, month: 8, today: date(2026, 8, 21), calendar: sundayFirst)
...
public struct MonthGrid: Equatable {
    ...
    public static func make(year: Int, month: Int, today: Date, calendar: Calendar) -> MonthGrid {
        ...
        guard let first = calendar.date(from: components),
            ...
            return MonthGrid(year: year, month: month, monthName: "",
                             weekdayTitles: titles(calendar), weeks: [])
        }
        ...
        formatter.locale = calendar.locale ?? Locale(identifier: "ru_RU")
        formatter.dateFormat = "LLLL"

        return MonthGrid(year: year, month: month,
                         monthName: formatter.string(from: first).uppercased(),
                         weekdayTitles: titles(calendar), weeks: weeks)
    }
    ...
    private static func titles(_ calendar: Calendar) -> [String] {
        let formatter = DateFormatter()
        formatter.calendar = calendar
        formatter.locale = calendar.locale ?? Locale(identifier: "ru_RU")
...
`UI/MonthView.swift`: сверху название месяца и года со стрелками влево-вправо, ниже строка из `weekdayTitles`, ниже семь колонок по `weeks`. Выходные — приглушённым цветом, сегодня — заливка кружком, `nil` — пустая клетка. Стрелки вызывают `shifted(by:)`. При каждом раскрытии панели сбрасывать на текущий месяц.
