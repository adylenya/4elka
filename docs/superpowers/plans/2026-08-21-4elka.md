...
### Task 26: Окно настроек

Пункт «Настройки…» из Task 25 должен открывать настоящее окно. Настраивается то, что уже есть в `Config` и что владелец захочет крутить: пороги заряда, город погоды, время жизни карточки, какие поставщики карточек включены.
