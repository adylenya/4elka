...
- [ ] **Step 6: Собрать в делегате и проверить руками**
...
public final class ChelkaAppDelegate: NSObject, NSApplicationDelegate {
    ...
    public func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        probe.layer?.backgroundColor = NSColor.systemPink.withAlphaComponent(0.7).cgColor
...
1. Наведение мышью на челку — появляется розовый прямоугольник ровно по челке, без смещения.
...
```bash
git add Sources Tests
git commit -m "feat: панель над челкой, зона-триггер и автомат состояний"
```
