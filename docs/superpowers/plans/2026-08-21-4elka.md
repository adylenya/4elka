...
- **Сборка обязана быть без предупреждений, включая тестовый таргет.** Мерить так:
  ```bash
  rm -rf .build && ./scripts/test.sh 2>&1 | grep -c "warning:"
  ```
  ⚠️ **`swift build` для этого не годится** — он собирает только исходники и не видит предупреждений в тестах. Контроллер полсессии мерил именно так и уверенно докладывал «ноль», пока предупреждения в тестах жили незамеченными. Повторный запуск без `rm -rf .build` тоже врёт: перекомпилировать нечего, и он покажет обманчивый ноль.
  Работа с AppKit из неизолированного контекста даёт предупреждения десятками — типы, которые трогают окна и вью, помечаются `@MainActor`, а не подавляются.
- **Строгая многопоточность Swift 6.** Тулчейн компилирует в режиме Swift 6 без послаблений, поэтому тип, который тест объявляет как `private let` на уровне файла, обязан быть `Sendable`. Для наших типов состояния это бесплатно: они хранят только значения. Если компилятор ругается «is not concurrency-safe because non-'Sendable' type…» — правильный ответ добавить `Sendable` к типу, а не переносить объявление внутрь теста.
...
- ⚠️ **Константы из `Config` в задание НЕ попадают.** Генератор заданий вырезает только раздел своей задачи, а `Config` живёт в общей части плана. Поэтому, добавляя константу для задачи, её значение надо **продублировать в текст самой задачи** — иначе исполнитель честно придумает своё число, и план с кодом разойдутся молча. На этом уже попались дважды: порог отказов плеера и ширина крыльев челки.
...
- **🚫 БЕЗ ЯВНОГО «ГО» ВЛАДЕЛЬЦА ЗАПРЕЩЕНО:** запускать приложение любым способом; писать в системный буфер обмена (`pbcopy`, `osascript ... set the clipboard`, `screencapture -c`); снимать экран; посылать синтетические события мыши и клавиатуры. Владелец работает за этой же машиной, и проверки лезли ему в буфер и на экран — его слова: «мешаешь работать». Он готов дать «го» и отойти, но спрашивать надо **до**, а не после.
  Автотесты через `./scripts/test.sh` под запрет не попадают: они работают на подделках и системного буфера не касаются.
- **Всё, что требует живого приложения, идёт в отчёт разделом «Нужно го владельца»** — списком, что посмотреть и какими действиями. Контроллер соберёт эти списки по всем задачам и попросит «го» один раз. Честное «требует го» — правильный результат, а не недоделка; выдавать непроверенное за проверенное нельзя.
- **Запущенное приложение видно человеку за машиной.** `swift run Chelka` рисует панель на настоящем экране владельца, поверх его работы. Запускать только когда это правда нужно для проверки, **гасить сразу после**, и никогда не оставлять запущенным к моменту отчёта. В отчёте писать, что приложение погашено.
- **Гасить только СВОЙ экземпляр, и только по имени исполняемого файла.** Правильно:
  ```bash
  kill -9 $(ps -eo pid,command | awk '/\.build\/[^ ]*\/Chelka( |$)/ && /<путь к твоему воркри>/ {print $1}')
  ```
  **Нельзя `pkill -f "4elka.*/Chelka"`**: этот шаблон совпадает и с процессами компилятора (в их командной строке есть и путь проекта, и имя модуля), и уже один раз убил сборку соседнего агента. В проекте параллельно работают несколько агентов в своих воркри — чужие процессы не твои.
- **Автозапуск при входе НЕ включать во время проверок.** Замерено: регистрация проходит даже из дерева сборки и создаёт настоящий элемент входа, указывающий на одноразовый воркри. Если включил для проверки — обязательно снять и подтвердить, что снят (`sfltool dumpbtm | grep -i 4elka` должен молчать).
- **Тесты запускаются ТОЛЬКО через `./scripts/test.sh`, а не через `swift test`.** Замерено на целевой машине: в Command Line Tools нет модуля `XCTest`, а `Testing.framework` есть на диске, но SwiftPM не знает путей до него и до его `lib_TestingInterop.dylib`. Скрипт подставляет пути, выводя их из `xcode-select -p`. Голый `swift test` падает с `no such module 'Testing'`, а попытка обойти это иначе давала **ложно-зелёный результат: ноль запущенных тестов при коде выхода 0**. Проверено: со скриптом зелёные тесты дают 0, падающий тест даёт 1, `--filter` работает. Xcode не нужен.
- **Тесты:** сначала падающий тест, потом реализация. Цель покрытия 80%+. Вьюхи не покрываются.
- ⚠️ **Настройка, добавленная в окно, обязана быть подключена той же задачей.** Замерено 24.08: из шестнадцати настроек применялись четыре, остальные крутились и не делали ничего — человек видит ручку и верит ей. Раздел, который нечем подключить, в окно не добавляется вовсе: отсутствующая настройка честна, а неподключённая врёт. Проверять надо не «значение сохранилось», а «поведение изменилось» — тест на сохранение существовал и не поймал ничего.
...
4elka/
  Package.swift
  Makefile
  scripts/test.sh                      обвязка для тестов: подставляет пути к Testing.framework
  .gitignore
...
- [ ] **Step 3: Создать обвязку для запуска тестов**

Без неё тесты не запускаются вообще. Замерено на целевой машине: в Command Line Tools нет модуля `XCTest`, а `Testing.framework` на диске есть, но SwiftPM не знает путей ни до него, ни до его `lib_TestingInterop.dylib` — голый `swift test` падает с `no such module 'Testing'`.
...
Если вместо этого видишь `no such module 'Testing'` — обвязка из шага 3 не подцепилась: проверь, что `xcode-select -p` отдаёт `/Library/Developer/CommandLineTools` и что по этому пути есть `Library/Developer/Frameworks/Testing.framework`. **Не подпирай это `unsafeFlags` в `Package.swift`** — такой путь уже проверялся и дал ложно-зелёный результат: сборка проходила, тестов запускалось ноль, код выхода был 0. Если обвязка не работает — останавливайся и сообщай.

- [ ] **Step 6: Реализовать `Config`**
...
public enum Config {
    ...
    public enum Battery {
        ...
        /// Полный заряд. Отдельной константой, а не числом в коде: правило «никаких
        /// магических чисел» действует и на физические величины.
    ...
    public enum Media {
        ...
        /// Задержка перед первой попыткой переподнять процесс потока.
        public static let restartDelayInitial: TimeInterval = 1
        /// Потолок задержки: дальше растить бессмысленно.
        public static let restartDelayMax: TimeInterval = 30
        /// Прожил дольше этого — считаем запуск удачным и сбрасываем задержку.
        public static let healthyRunGrace: TimeInterval = 5
        /// Столько мгновенных смертей подряд означает, что адаптера нет или он
        /// сломан. Дальше не дёргаемся, а честно сообщаем «плеер недоступен».
        public static let maxImmediateFailures = 5
        /// Потолок недособранной строки. Поток без переводов строки не должен
        /// съесть всю память.
        public static let maxPendingBytes = 1 << 20
    }
    ...
    public enum Notch {
        ...
        /// На сколько фигура выходит за челку влево и вправо. В этих крыльях
        /// живёт короткое: значок приложения слева, состояние справа.
        public static let wingWidth: CGFloat = 64
        /// Радиус нижних углов фигуры, продолжающей челку. Должен совпадать
        /// со скруглением самой челки снизу, иначе стык виден.
...
- [ ] **Step 8: Убедиться, что тесты проходят и приложение собирается**
...
- [ ] **Step 9: Доказать, что тесты не врут**

Обязательный шаг, не перестраховка: предыдущая попытка запустить тесты на этой машине давала код выхода 0 при нуле запущенных тестов, и такую обвязку нельзя принимать на слово.

Временно добавь в `ConfigTests.swift` заведомо ложное ожидание:

```swift
@Test func harnessHonestyCheck() { #expect(Config.History.textLimit == 999999) }
```

Run: `./scripts/test.sh; echo "код выхода = $?"`
Expected: тест падает с указанием файла и строки, **код выхода не 0**.

Затем удали этот тест, убедись, что снова всё зелёное и код выхода 0. В отчёте приведи оба кода выхода — это и есть доказательство, что обвязка честная.

- [ ] **Step 10: Коммит**

```bash
git add Package.swift .gitignore Sources Tests scripts
git commit -m "chore: каркас проекта на SwiftPM с конфигом и обвязкой тестов"
```
...
Run: `./scripts/test.sh --filter NotchGeometryTests 2>&1 | tail -10`
...
Run: `./scripts/test.sh --filter NotchGeometryTests 2>&1 | tail -5`
Expected: PASS, 9 тестов (4 исходных плюс 5 на потолок буфера и политику перезапуска).
...
Run: `./scripts/test.sh --filter PanelStateTests 2>&1 | tail -10`
...
3б. **Провести мышью с нажатой кнопкой по верхней части раскрытой панели** — жест должен доходить до панели, а не упираться в невидимую зону-триггер. Это проверка того, что следующая задача с перетаскиванием вообще возможна.
...
Run: `./scripts/test.sh --filter DragMaterializerTests 2>&1 | tail -10`
Expected: FAIL — тип не найден.
...
Run: `./scripts/test.sh --filter DragMaterializerTests 2>&1 | tail -5`
...
Run: `./scripts/test.sh --filter HistoryStoreTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter HistoryStoreTests 2>&1 | tail -5`
...
Run: `./scripts/test.sh --filter IgnoreRulesTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter IgnoreRulesTests 2>&1 | tail -5`
...
@Test func blobNameCannotEscapeItsDirectory() throws {
    ...
    #expect(store.url(for: "../посторонний.txt").deletingLastPathComponent()
            == base.appendingPathComponent("blobs"))
}
...
Run: `./scripts/test.sh --filter "BlobStoreTests|HistoryIndexTests" 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter "BlobStoreTests|HistoryIndexTests" 2>&1 | tail -5`
Expected: PASS, 7 тестов.
...
> **`ActivityEvent` уже существует** — его создаёт задача 18 в файле
> `Sources/ChelkaCore/Activity/ActivityEvent.swift`, потому что уведомления о заряде
> оказались первым потребителем и без этого типа не компилировались. **Не объявляй его
> заново** — получишь двойное определение. Только прочти его и используй.
...
- Produces:
  ...
  public struct ActivityQueue: Equatable {
      ...
      /// Событие, отложенное из-за раскрытой панели. Только для заряда — см. ниже.
      public let deferred: ActivityEvent?
      public func submitting(_ e: ActivityEvent, now: Date, panel: PanelState) -> ActivityQueue
      public func ticking(now: Date, panel: PanelState) -> ActivityQueue
  }
  ```
...
@Test func suppressedWhilePanelExpanded() {
    ...
    #expect(q.deferred == nil)
}

@Test func batteryWarningIsDeferredNotLostWhilePanelExpanded() {
    // Правило уведомлений снимает взвод при выстреле, поэтому выброшенное
    // предупреждение о заряде не вернётся никогда. Его обязано отложить.
    let q = ActivityQueue().submitting(ev(.battery, "мало заряда"), now: t0, panel: .expanded)
    #expect(q.current == nil)
    #expect(q.deferred?.title == "мало заряда")
}

@Test func deferredWarningAppearsWhenPanelCloses() {
    let q = ActivityQueue()
        .submitting(ev(.battery, "мало заряда"), now: t0, panel: .expanded)
        .ticking(now: t0.addingTimeInterval(1), panel: .hidden)
    #expect(q.current?.title == "мало заряда")
    #expect(q.deferred == nil)
    #expect(q.deadline == t0.addingTimeInterval(1 + Config.Activity.duration))
}

@Test func deferredWarningWaitsWhilePanelStaysExpanded() {
    let q = ActivityQueue()
        .submitting(ev(.battery, "мало заряда"), now: t0, panel: .expanded)
        .ticking(now: t0.addingTimeInterval(10), panel: .expanded)
    #expect(q.current == nil)
    #expect(q.deferred?.title == "мало заряда")
}
...
@Test func tickHidesCardAfterDeadline() {
    let q = ActivityQueue().submitting(ev(.track, "трек"), now: t0, panel: .hidden)
    #expect(q.ticking(now: t0.addingTimeInterval(1), panel: .hidden).current != nil)
    #expect(q.ticking(now: t0.addingTimeInterval(Config.Activity.duration + 0.1), panel: .hidden).current == nil)
}

@Test func tickOnEmptyQueueIsNoOp() {
    #expect(ActivityQueue().ticking(now: t0, panel: .hidden) == ActivityQueue())
}
```

- [ ] **Step 2: Запустить и убедиться, что падает**

Run: `./scripts/test.sh --filter ActivityQueueTests 2>&1 | tail -10`
Expected: FAIL — типы не найдены.

- [ ] **Step 3: Проверить событие (создано задачей 18)**

Тип уже лежит в `Sources/ChelkaCore/Activity/ActivityEvent.swift`. Убедись, что он совпадает
с приведённым ниже, и **ничего не создавай**. Если расходится — приведи в соответствие,
но не дублируй объявление.
...
public struct ActivityQueue: Equatable {
    ...
    public func submitting(_ event: ActivityEvent, now: Date, panel: PanelState) -> ActivityQueue {
        guard panel != .expanded else {
            // Заряд нельзя просто выбросить. Правило уведомлений снимает взвод
            // при выстреле, поэтому подавленное предупреждение не «покажется
            // позже само» — оно исчезнет навсегда, и человек не узнает, что
            // наушники садятся. Откладываем до закрытия панели.
            // Трек и буфер выбрасывать можно: они про то, что и так видно.
            guard event.kind == .battery else { return self }
            return ActivityQueue(current: current, deadline: deadline, deferred: event)
        }
    ...
    public func ticking(now: Date, panel: PanelState) -> ActivityQueue {
        // Панель закрылась, а отложенное предупреждение ждёт — показываем.
        if panel != .expanded, let waiting = deferred {
            return ActivityQueue(current: waiting,
                                 deadline: now.addingTimeInterval(Config.Activity.duration),
                                 deferred: nil)
        }
        guard let deadline, now >= deadline else { return self }
        return ActivityQueue(current: nil, deadline: nil, deferred: deferred)
    }
}
```
...
Run: `./scripts/test.sh --filter ActivityQueueTests 2>&1 | tail -5`
Expected: PASS, 7 тестов.
...
Run: `./scripts/test.sh --filter PasteboardWatcherTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter PasteboardWatcherTests 2>&1 | tail -5`
Expected: PASS, 7 тестов.
...
Run: `./scripts/test.sh --filter ClipboardCoordinatorTests 2>&1 | tail -10`
Expected: FAIL.
...
public final class ActivityCenter: ObservableObject {
    ...
    public func tick(now: Date = Date()) { queue = queue.ticking(now: now, panel: panelState()) }
}
...
Run: `./scripts/test.sh --filter ClipboardCoordinatorTests 2>&1 | tail -5`
...
Run: `./scripts/test.sh --filter HistorySearchTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter HistorySearchTests 2>&1 | tail -5`
Expected: PASS, 7 тестов.
...
> **Два контракта от задачи 4, которые обязана соблюсти эта.** Первый: подготовленные
> для перетаскивания файлы **никто не удаляет**, поэтому материализовать надо лениво —
> в момент начала жеста, а не заранее — и заводить периодическую чистку временного
> каталога. Второй: источник перетаскивания сам ничего не материализует, поэтому
> вызывающий обязан **поймать ошибку по каждому элементу отдельно** и выбросить его
> из жеста: иначе пропавший с диска блоб сломает перетаскивание целиком вместо
> одного элемента.
...
> **Константы этой задачи** (выбраны исполнителем 21.08, значения в коде —
> дублируются здесь, потому что раздел `Config` в задание не попадает):
> `Config.Drag`: время жизни временного файла жеста **час**, длина имени из текста
> **40** символов, формат штампа `yyyy-MM-dd HH-mm-ss` (двоеточие в имени файла
> недопустимо). `Config.HistoryGrid`: плитка **92**, зазор между плитками **8**,
> отступ сетки **10**, зазор строк **8**, внутренний зазор плитки **4**, рамка
> выделения **2**, скругление плитки **8**, строк текста на плитке **2**,
> иконка файла **40**.
...
Run: `./scripts/test.sh --filter NowPlayingTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter NowPlayingTests 2>&1 | tail -5`
...
Run: `./scripts/test.sh --filter MediaCommandTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter MediaCommandTests 2>&1 | tail -5`
...
// его через `let (coordinator, source, events) = make()
    defer { _ = coordinator }`. Захват в координаторе слабый, и
...
Run: `./scripts/test.sh --filter MediaCoordinatorTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter MediaCoordinatorTests 2>&1 | tail -5`
Expected: PASS, 7 тестов.
...
Run: `./scripts/test.sh --filter MonthGridTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter MonthGridTests 2>&1 | tail -5`
Expected: PASS, 9 тестов.
...
Run: `./scripts/test.sh --filter DeviceChargeTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter DeviceChargeTests 2>&1 | tail -5`
...
Run: `./scripts/test.sh --filter BatteryAlertsTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter BatteryAlertsTests 2>&1 | tail -5`
Expected: PASS, 9 тестов.
...
Run: `./scripts/test.sh --filter WeatherTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter WeatherTests 2>&1 | tail -5`
Expected: PASS, 9 тестов.
...
Run: `./scripts/test.sh --filter TransformationsTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter TransformationsTests 2>&1 | tail -5`
...
Run: `./scripts/test.sh --filter TextRecognitionTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter TextRecognitionTests 2>&1 | tail -5`
Expected: PASS, 5 тестов.
...
> **Константы этой задачи** (выбраны исполнителем 21.08, дублируются здесь, потому
> что раздел `Config` в задание не попадает): `Config.Shelf` — высота полосы полки в
> раскрытой панели **128** (плитка 92 плюс заголовок с кнопками плюс зазор), зазор
> между заголовком и плитками **6**, толщина рамки подсветки зоны приёма **2**,
> скругление этой рамки **10**, строк имени файла на плитке **2**. Путь хранения —
> `AppPaths.shelf` = `support/shelf.json`. Размеры плиток переиспользованы из
> `Config.HistoryGrid`, новых значений там не появилось.
>
> **Решения, принятые исполнителем и принятые мной:** полка пишется на диск сразу,
> без задержки — её меняет рука человека, схлопывать нечего, а потерять обидно.
> Квоты у полки нет вовсе: элементы живут до явного удаления, полка может расти
> неограниченно — осознанный выбор. Симлинки при дедупликации не разворачиваются,
> потому что разворот требует обращения к диску, а дедупликация обязана оставаться
> чистой функцией. Обычный клик по плитке полки **выделяет**, а не копирует в буфер
> (в истории клик копирует): с полки файлы уносят жестом или отправляют кнопкой.
...
Run: `./scripts/test.sh --filter ShelfStoreTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter ShelfStoreTests 2>&1 | tail -5`
Expected: PASS, 8 тестов.
...
> ⚠️ **Сочетание изменено 21.08 после слияния: не ⌘⇧V, а ⌃⌥V.** ⌘⇧V — это системная
> «вставка без форматирования» почти во всех редакторах и браузерах, а глобальная
> регистрация забирает сочетание себе, то есть приложение ломало бы то, чем человек
> пользуется ежедневно. Код и тесты уже приведены к ⌃⌥V; примеры ниже сохранены как
> есть, читать их надо с этой поправкой. Тест закрепляет **требование** («сочетание
> по умолчанию не равно вставке без форматирования»), а не конкретные клавиши.
...
Run: `./scripts/test.sh --filter HotkeyTests 2>&1 | tail -10`
Expected: FAIL.
...
Run: `./scripts/test.sh --filter HotkeyTests 2>&1 | tail -5`
Expected: PASS, 4 теста.
...
**Порядок остатка изменён 21.08:** Task 25 (иконка в строке меню) идёт **раньше** остальных
остатков. Причина не в архитектуре, а в том, что приложение уже случайно запустилось на живой
машине владельца, и выключить его было нечем: иконки в доке нет, панель живёт над челкой.
Аварийный выход важнее следующей фичи.

**Задача 31 добавлена 21.08 ночью** по находкам ревью оконной подсистемы: клик по
челке, скорее всего, не работает вообще (панель накрывает зону-триггер), карточка
затирает раскрытую панель без восстановления, геометрия не пересчитывается на смену
экрана. Идёт после слияния задач 22, 23 и 26 — все они правят делегат приложения.

**Задача 30 добавлена 21.08 ночью:** подсистемы 12–21 оказались не подключены к
приложению — сборку раскрытой панели не делал никто. Она идёт после задачи 11 и до
задачи 23. Правило на будущее: **задача, создающая вьюху, обязана либо подключить её,
либо назвать номер той задачи, что подключит.**
...
### Task 25: Иконка в строке меню — аварийный выход

Приложение живёт без иконки в доке, а панель появляется только над челкой. Значит **у него нет ни одного способа быть выключенным**: человек, случайно его запустивший, остаётся с панелью, которую нечем закрыть. Именно это и произошло на живой машине 21.08. Задача маленькая и должна быть сделана раньше остальных остатков.

**Files:**
- Create: `Sources/ChelkaCore/App/StatusItemController.swift`
- Modify: `Sources/ChelkaCore/App/AppDelegate.swift`
- Test: `Tests/ChelkaCoreTests/StatusMenuTests.swift`

**Interfaces:**
- Consumes: `PanelState` из Task 3
- Produces:
  ```swift
  ...
  public enum StatusMenu {
      /// Пункты меню для текущего состояния. Чистая функция — её и проверяем.
...
Проверяем то, что можно проверить без окон: состав меню и подписи.
...
Run: `./scripts/test.sh --filter StatusMenuTests 2>&1 | tail -10`
Expected: FAIL — типы не найдены.

- [ ] **Step 3: Реализовать состав меню**
...
public enum StatusMenu {
    ...
    public static func title(for action: StatusMenuAction, launchesAtLogin: Bool) -> String {
        ...
        case .toggleLaunchAtLogin:
            // Подпись про действие, а не про состояние.
...
Run: `./scripts/test.sh --filter StatusMenuTests 2>&1 | tail -5`
Expected: PASS, 5 тестов.

- [ ] **Step 5: Реализовать саму иконку**
...
- [ ] **Step 6: Подключить в делегате**

Создать `StatusItemController` в `applicationDidFinishLaunching`, вызывать `refresh` при каждой смене состояния панели, и обработать действия: показать панель, открыть настройки (пока пустая заглушка-окно — настоящее окно делает Task 26), переключить автозапуск через `SMAppService.mainApp`, выключить через `NSApp.terminate(nil)`.

Автозапуск требует, чтобы приложение лежало в `/Applications` — из дерева сборки не сработает, и это нормально. Ошибку регистрации писать в лог, а не глотать.

- [ ] **Step 7: Проверить руками**

Run: `swift run Chelka`

1. В строке меню появилась иконка.
2. Клик по ней открывает меню с четырьмя пунктами.
3. **«Выключить 4elka» действительно завершает приложение** — панель исчезает, процесс умирает. Это главная проверка задачи.
4. При раскрытой панели пункт «Показать панель» из меню пропадает.

**После проверки обязательно погасить приложение**, если оно почему-то осталось жить: `pkill -f "4elka.*/Chelka"`. Оно рисуется на настоящем экране владельца.

- [ ] **Step 8: Коммит**

```bash
git add Sources Tests
git commit -m "feat: иконка в строке меню с выходом, показом панели и автозапуском"
```

---

### Task 26: Окно настроек — полное

Пункт «Настройки…» из Task 25 должен открывать настоящее окно, и настраиваться должно **всё, что имеет смысл крутить**, а не выборочные три поля. Владелец сказал прямо: «фул настройку надо сделать».

Разделы окна и что в них:

| Раздел | Настройки |
|---|---|
| Буфер обмена | сколько хранить текста, картинок, файлов; потолок размера картинки; список приложений, из которых не записывать |
| Карточки | время жизни; отдельные тумблеры на три источника — трек, буфер, заряд |
| Заряд | нижний порог, верхний порог, гистерезис; показывать ли айфон |
| Погода | координаты (с полем поиска города вместо ввода чисел), интервал обновления, порог устаревания |
| Календарь | первый день недели: по системе или задать вручную |
| Плеер | показывать ли обложку, показывать ли полосу позиции |
| Поведение | хоткей открытия панели; открывать ли панель по наведению или только по клику; автозапуск при входе |
| Данные | где лежит история, её размер, кнопка «Очистить историю» с подтверждением |

Раздел «Данные» обязателен: при первом запуске приложение подхватывает то, что уже лежит в буфере, и человек должен иметь возможность стереть это, не лазая в терминал.

**Files:**
- Create: `Sources/ChelkaCore/Core/Settings.swift`
- Create: `Sources/ChelkaCore/UI/SettingsView.swift`
- Modify: `Sources/ChelkaCore/App/AppDelegate.swift`
- Test: `Tests/ChelkaCoreTests/SettingsTests.swift`
...
- Produces:
  ...
  public struct Settings: Equatable, Codable, Sendable {
      ...
      /// Приводит значения в допустимые пределы. Настройки редактирует человек,
      /// а значит на входе может оказаться что угодно.
...
Run: `./scripts/test.sh --filter SettingsTests 2>&1 | tail -10`
Expected: FAIL — типы не найдены.

- [ ] **Step 3: Реализовать модель и хранение**

Значения по умолчанию берутся из `Config` — он остаётся источником истины, а настройки его перекрывают. `sanitized()` ставит пороги в правильном порядке, загоняет время жизни карточки в разумные пределы и проверяет, что координаты лежат на Земле. Хранение — JSON рядом с историей, через `AtomicFile`; битый или отсутствующий файл даёт значения по умолчанию, а не крэш.

- [ ] **Step 4: Проверить, что тесты проходят**

Run: `./scripts/test.sh --filter SettingsTests 2>&1 | tail -5`
Expected: PASS, 6 тестов.

- [ ] **Step 5: Нарисовать окно**

`UI/SettingsView.swift`: `Form` с полями по разделам — заряд, карточки, погода. Обычное окно (не панель над челкой), открывается по пункту меню, закрывается как любое окно. Тема системная, семантические цвета, никакого хардкода.

Сохранение при изменении, с прогоном через `sanitized()` перед записью.

- [ ] **Step 6: Проверить руками**

Run: `swift run Chelka`

1. Пункт «Настройки…» открывает окно.
2. Изменение порога заряда сохраняется и переживает перезапуск приложения.
3. Ввод бессмысленного значения (пороги наоборот, время жизни −5) приводится к разумному, а не ломает приложение.

**После проверки погасить приложение:** `pkill -f "4elka.*/Chelka"`.

- [ ] **Step 7: Коммит**

```bash
git add Sources Tests
git commit -m "feat: окно настроек с проверкой значений и хранением на диске"
```

### Task 27: Челка не перекрывает содержимое, карточка не крадёт фокус

Оба дефекта замечены владельцем на живом приложении и оба мои. Задача срочная: второй мешает человеку печатать.

**Files:**
- Create: `Sources/ChelkaCore/Notch/NotchLayout.swift`
- Modify: `Sources/ChelkaCore/Notch/NotchPanel.swift`
- Modify: `Sources/ChelkaCore/App/AppDelegate.swift`
- Test: `Tests/ChelkaCoreTests/NotchLayoutTests.swift`

**Interfaces:**
- Consumes: `NotchGeometry` из Task 2, `PanelState` из Task 3
- Produces:
  ```swift
  public struct NotchLayout: Equatable, Sendable {
      ...
      /// Куда ставить выезжающую карточку: её верх начинается ПОД челкой.
      public static func cardFrame(size: CGSize, geometry: NotchGeometry) -> CGRect
      /// Раскладка внутри панели заданного размера (координаты вью, начало слева снизу).
...
Замеренные значения целевой машины: челка 220×38, центр по X = 1028, экран 2056×1329.
...
Run: `./scripts/test.sh --filter NotchLayoutTests 2>&1 | tail -10`
Expected: FAIL — типы и методы не найдены.
...
- [ ] **Step 4: Запретить панели брать фокус во всех состояниях кроме раскрытого**
...
```swift
    ...
    public static func allowsKeyboard(in state: PanelState) -> Bool { state == .expanded }
...
И в конструкторе добавить `becomesKeyOnlyIfNeeded = true`.

- [ ] **Step 5: Проверить, что тесты проходят**

Run: `./scripts/test.sh --filter NotchLayoutTests 2>&1 | tail -5`
Expected: PASS, 6 тестов.

- [ ] **Step 6: Применить в делегате**

В `apply(_:)`: перед каждым переходом звать `panel.setKeyboardAllowed(NotchPanel.allowsKeyboard(in: machine.state))`, и **только** в раскрытом состоянии `panel.makeKey()`.

Для состояния `.activity` ставить окно по `NotchLayout.cardFrame(size:geometry:)`, а не по `resize(to:geometry:)`.

Содержимое панели раскладывать по `NotchLayout.inPanel`: короткое (время, погода, заряд) в полосы слева и справа от челки, основное — в тело.

Заодно перенести `refreshStatusItem()` и `setInteractive` **до** раннего выхода `guard let panel else { return }`: сейчас при отсутствующей панели меню и зона-триггер не обновляются.

- [ ] **Step 7: Проверить руками — это главное**

Собрать и запустить **коротко**, потом сразу погасить.

1. **Карточка целиком видна** — её первая строка не съедена челкой.
2. **Печатать в редакторе, пока выезжают карточки** (скопировать что-нибудь несколько раз). **Ни один символ не должен пропасть, фокус не должен уходить.** Это главная проверка.
3. Раскрыть панель — в полосах слева и справа от челки видно короткое, основное ниже челки, ничего не спрятано.
4. В раскрытой панели набор текста работает (там будет поиск).

Владелец сидит за машиной и уже дважды жаловался на всплывающую панель — держи приложение запущенным считанные секунды.

- [ ] **Step 8: Коммит**

```bash
git add Sources Tests
git commit -m "fix: раскладка обходит челку, клавиатуру берёт только раскрытая панель"
```

### Task 28: Git по-человечески

Сейчас репозиторий живёт локально, историю писали двадцать с лишним агентов, и remote нет вовсе. Надо привести в вид, в котором проект не стыдно открыть через год или показать другому человеку.
...
- [ ] **Step 1: `README.md` — первое, что видит человек**

Не пересказ плана, а короткий и честный документ: что это за приложение и зачем, скриншот главного (иконка в строке меню, раскрытая панель, карточка), как собрать (`make app`, `make install`), какие внешние утилиты необязательны (`ideviceinfo` для заряда айфона), где лежат данные, как выключить и как удалить. Ссылки на спеку и план — в конце, для тех, кто полезет глубже.

Отдельным разделом — **что приложение НЕ делает и почему**: не требует ни одного системного разрешения, не перехватывает хоткеи, не читает экран. Это его главное свойство, и о нём стоит сказать первым, а не прятать.

- [ ] **Step 2: `.gitattributes`**

`* text=auto eol=lf` плюс явная разметка бинарных фикстур, чтобы git не пытался считать их текстом и не портил при переносе.

- [ ] **Step 3: `CHANGELOG.md`**

Первая запись — версия 1.0 с перечислением того, что вошло. Дальше по одной записи на заметное изменение. Формат простой: версия, дата, список человеческим языком, без внутренних номеров задач.

- [ ] **Step 4: Причесать `.gitignore`**

Сверить с фактическим состоянием: `.build`, `.swiftpm`, `build`, `vendor/build`, `vendor/mediaremote-adapter`, `.claude/worktrees`, `.superpowers`, `*.app`, `.DS_Store`. Убрать то, чего в проекте не бывает, добавить то, что мусорится на практике.

- [ ] **Step 5: Отчистить оставшиеся воркри и ветки**

`git worktree list` и `git branch` не должны показывать ничего от агентов. Удалить брошенные воркри, слить или удалить их ветки.

- [ ] **Step 6: Проставить тег версии**

`git tag -a v1.0 -m "первая рабочая версия"` — но **только после** того, как финальное ревью пройдено и приложение проверено глазами.

- [ ] **Step 7: Remote — это шаг владельца**

⚠️ Создать репозиторий и авторизоваться агент не может: `gh auth status` показывает, что вход в GitHub не выполнен. Владельцу нужно сделать одно из двух:

```bash
gh auth login          # затем: gh repo create 4elka --private --source=. --push
```
либо создать репозиторий в интерфейсе GitHub и выполнить:
```bash
git remote add origin git@github.com:<логин>/4elka.git
git push -u origin main --tags
```

Пока remote нет, история живёт только на этой машине — и это стоит сказать вслух, а не оставить как незамеченный риск.

- [ ] **Step 8: Коммит**

```bash
git add README.md .gitattributes CHANGELOG.md .gitignore
git commit -m "docs: README, changelog и приведение репозитория в порядок"
```

---

> **Хвост задачи 28, который закрывает эта:** в `README.md` раздел про сборку
> описывает то, что работает сейчас (`make adapter`, `make build`, `make run`,
> `make test`). Когда появятся цели `make app` и `make install`, раздел установки
> надо переписать под них — иначе README обещает то, чего нет.

### Task 29: Руководство со скриншотами

Владелец просил «гайд как пользоваться с скриншотами». Приложение прячется в челке и не имеет иконки в доке, поэтому без картинок объяснить, где что, почти невозможно.

**Files:**
- Create: `docs/guide/README.md`
- Create: `docs/guide/images/` (скриншоты)

**Что сделать:**

- [ ] **Step 1: Снять настоящие скриншоты**

Не рисунки и не описания словами — снимки живого приложения через `screencapture`. Нужны:

1. Иконка в строке меню с раскрытым меню.
2. Челка в покое (ничего не видно — это тоже часть объяснения).
3. Карточка после копирования текста.
4. Карточка после скриншота, с миниатюрой.
5. Раскрытая панель: история, вкладки, плеер, нижняя полоса с календарём, зарядом и погодой.
6. Перетаскивание нескольких скриншотов наружу.
7. Окно настроек.

**Приложение рисуется на настоящем экране владельца, и он уже трижды жаловался.** Поэтому: подготовить всё заранее, снять серию за один короткий запуск, сразу погасить. Гасить только свой экземпляр по имени исполняемого файла — не широким шаблоном.

Кадрировать так, чтобы не попадали чужие окна, переписки и личные данные. Если в кадр попало лишнее — снять заново, а не замазывать.

- [ ] **Step 2: Написать руководство**

Порядок — по тому, как человек этим пользуется, а не по устройству программы:

- **Первый запуск** — где искать иконку, как открыть панель, как выключить.
- **Скриншоты** — снимаешь как обычно с зажатым Control, стек последних доступен в панели, вытащить можно один или пачкой.
- **Буфер обмена** — история, поиск, закрепление, что в историю не попадает и почему (пароли из менеджеров).
- **Музыка** — карточка на смену трека и пауза-плей, кнопки в панели.
- **Календарь, заряд, погода** — что показывается и откуда берётся; отдельно честно: заряд айфона только по кабелю.
- **Настройки** — по разделам.
- **Если что-то не работает** — плеер молчит (собрать адаптер), заряд айфона не виден (кабель, доверенное устройство, `ideviceinfo`), панель не появляется на внешнем мониторе (там нет челки, работает плашка по центру).

Язык — обычный, как в разговоре. Никаких имён типов и файлов: это руководство для пользователя, а не для того, кто читает код.

- [ ] **Step 3: Проверить руководство на себе**

Пройти по нему с чистого листа, как будто впервые видишь приложение. Всё, что придётся домысливать, — дописать. Всё, что не подтверждается скриншотом, — либо снять, либо убрать из текста.

- [ ] **Step 4: Коммит**

```bash
git add docs/guide
git commit -m "docs: руководство пользователя со скриншотами"
```

---

### Task 30: Сборка раскрытой панели — подсистемы наконец подключаются

**Зачем эта задача появилась (21.08, ночь).** Обнаружено при живой проверке: плеер,
календарь, погода и заряды **написаны, покрыты тестами и никуда не подключены**.
`AdapterPaths` и `MediaRemoteBridge` не создаёт никто, `MediaCoordinator` в
`AppDelegate` не появляется, `PlayerView`, `MonthView`, `WeatherView` не используются
ни в одном месте — раскрытая панель до сих пор рисует заглушку `Text("4elka")`.
В плане не было задачи, которая сводит подсистемы воедино: задачи 12–21 создавали
вьюхи и логику, а сборку не делал никто. Отсюда и правило на будущее: **задача,
создающая вьюху, обязана либо подключить её, либо назвать ту, что подключит.**

Идёт **после задачи 11** (сетка истории — главный житель панели) и **до задачи 23**
(хоткею нужна готовая панель).

**Files:**
- Create: `Sources/ChelkaCore/UI/PanelContentView.swift` — раскладка раскрытой панели
- Create: `Sources/ChelkaCore/UI/DevicesView.swift` — список зарядов, если его ещё нет
- Create: `Sources/ChelkaCore/App/ServiceContainer.swift` — сборка зависимостей в одном месте
- Modify: `Sources/ChelkaCore/App/AppDelegate.swift` — заменить заглушку на настоящее содержимое
- Modify: `Sources/ChelkaCore/Media/MediaRemoteBridge.swift` — только если понадобится для абсолютного пути
- Test: `Tests/ChelkaCoreTests/AdapterPathsTests.swift`
- Test: `Tests/ChelkaCoreTests/PanelSectionsTests.swift`

**Что уже сделано задачей 11 (21.08):** сетка истории подключена к раскрытому
состоянию панели, и содержимое пересобирается только на переходе в раскрытое
состояние и обратно — не на каждый вызов `apply`, иначе обнулялись бы строка
поиска, выделение и фокус. Этой задаче остаётся добавить остальные разделы, не
сломав это правило.

**Что должно оказаться в раскрытой панели** (сверху вниз, в теле под челкой):
плеер (или строка «ничего не играет»), сетка истории с поиском и вкладками из задачи 11,
и нижняя полоса: календарь-месяц, погода, заряды устройств. В крыльях слева и справа
от челки — короткое: время и значок заряда.

- [ ] **Step 1: Падающие тесты на путь к адаптеру**

Замерено на живой машине 21.08: **адаптер грузится только по абсолютному пути**.
С относительным perl падает с `Failed to load framework`, и плеер молча мёртв.
`AdapterPaths.developmentTree(projectRoot:)` принимает корень снаружи, и никто не
гарантирует, что он абсолютный.

```swift
@Test func developmentTreePathsAreAbsolute() {
    let paths = AdapterPaths.developmentTree(projectRoot: URL(fileURLWithPath: "относительный"))
    // Замерено: perl не грузит фреймворк по относительному пути и молча оставляет
    // плеер мёртвым. Значит собирать путь надо от абсолютного корня.
    #expect(paths.script.path.hasPrefix("/"))
    #expect(paths.framework.path.hasPrefix("/"))
}

@Test func resolvedPathsPreferBundleOverDevelopmentTree() {
    // В собранном .app адаптер лежит в ресурсах бандла; в дереве сборки — в vendor.
    // Порядок важен: установленное приложение не должно смотреть в чужой воркри.
    #expect(AdapterPaths.resolutionOrder == [.bundle, .developmentTree])
}
```

- [ ] **Step 2: Разрешение пути к адаптеру**

`AdapterPaths.resolved()`: сначала `bundled(in: .main)`, затем дерево разработки от
**абсолютного** корня. Корень дерева выводить из пути исполняемого файла
(`Bundle.main.executableURL`, три уровня вверх от `.build/<конфигурация>/Chelka`),
а не из текущего каталога процесса: текущий каталог зависит от того, откуда запустили.
Если ни один путь не существует — вернуть `nil`, и плеер честно показывает
«плеер недоступен», а не пустое место.

- [ ] **Step 3: Контейнер сервисов**

Все долгоживущие объекты создаются в одном месте и хранятся владельцем:
история и блобы, наблюдатель буфера, координатор плеера, поставщик зарядов,
погода, предупреждения о заряде. `AppDelegate` берёт их оттуда, а не собирает по
месту. Причина: сейчас в `AppDelegate` уже больше 300 строк, и добавление шести
подсистем по месту сделает его нечитаемым.

> **Обязательное требование, добытое замером 21.08:** `AppDelegate` обязан гасить
> мост плеера в `applicationWillTerminate`. Замерено: процесс адаптера переживает
> смерть хозяина и умирает лишь при следующей попытке записи в закрытую трубу —
> то есть висит, пока не сменится трек, а при паузе неограниченно долго.
> Уборка брошенных процессов при старте моста уже есть (`AdapterOrphans`), но это
> страховка на случай `kill -9`, а не замена вежливому завершению.

- [ ] **Step 4: Раскладка панели**

`PanelContentView` строится по `NotchLayout.inPanel`: крылья и тело отдельно.
Никаких новых цветов — только семантические, как везде. Разделы, которые нечем
наполнить (нет погоды, нет зарядов, плеер недоступен), показывают строку-заглушку,
а не исчезают: исчезающие разделы дёргают раскладку при каждом обновлении.

Тест на состав разделов держать чистой функцией от состояния, без окон:

```swift
@Test func sectionsAreStableWhenDataIsMissing() {
    // Разделы не пропадают из панели при отсутствии данных: иначе раскладка
    // прыгает каждые несколько секунд.
...
- [ ] **Step 5: Подключение к автомату состояний**

Раскрытие панели сбрасывает календарь на текущий месяц (правило из задачи 16).
Плеер запускается один раз при старте приложения, а не при каждом раскрытии:
поток адаптера — долгоживущий процесс, и перезапуск его на каждое раскрытие
означал бы новый процесс perl несколько раз в минуту.

- [ ] **Step 6: Проверить, что тесты проходят и предупреждений нет**

```bash
rm -rf .build && ./scripts/test.sh 2>&1 | grep -c "warning:"
```
Ожидается ноль. Тестов должно стать больше, ни одного упавшего.

- [ ] **Step 7: Живая проверка — только по «го» владельца**

В отчёт разделом «Нужно го владельца», не выполнять самому:
1. Раскрыть панель — видны плеер с обложкой, сетка истории, календарь, погода, заряды.
2. При играющей музыке кнопки переключают трек, полоса позиции движется.
3. Наушники и айфон видны в списке зарядов (замерено 21.08: разборщики отдают
   AirPods 3 и айфон по кабелю правильно, но в панели это никто не видел).
4. Стрелки календаря переключают месяц, при повторном раскрытии — текущий.

- [ ] **Step 8: Коммит**

```bash
git add Sources Tests
git commit -m "feat: раскрытая панель собрана — плеер, история, календарь, погода, заряды"
```

**Константы, добавленные сборкой панели 24.08.** Дублируются здесь, потому что
раздел `Config` в задания агентам не попадает.

`Config.Panel` — раскрытая панель как сумма разделов, а не круглое число:
`sectionSpacing = 10` (зазор между разделами), `verticalSections = 4` (плеер,
сетка истории, полка, нижняя полоса — зазоров на один меньше),
`playerHeight = Media.artworkSide + HistoryGrid.padding * 2` (**76**),
`calendarWidth = Calendar.dayCellHeight * 8 + HistoryGrid.padding * 2` (**244**),
`bottomBarHeight = Calendar.gridHeight + Calendar.headerHeight +
Calendar.weekdayRowHeight + Calendar.sectionSpacing * 2` (**242**),
`contentWidth = HistoryGrid.tileSide * 7 + tileSpacing * 6 + padding * 2` (**712**),
`verticalPadding = HistoryGrid.padding` (**10**, учтён в общей высоте — не
учтённый, он отбирал место у сетки истории), `contentHeight = playerHeight +
HistoryGrid.minHeight + Shelf.stripHeight + bottomBarHeight + sectionSpacing * 3
+ verticalPadding * 2` (**672**), `rowSpacing = 6`, `deviceSymbolWidth = 22`.

`Config.Notch.expandedSize` больше не `640×340`, а `Panel.contentWidth ×
Panel.contentHeight` (**712×672**): в 340 точек не влезали ни плеер, ни нижняя
полоса, и они обрезались нижним краем.

`Config.HistoryGrid`: `searchRowHeight = 24`, `tabRowHeight = 24`,
`minHeight = searchRowHeight + tabRowHeight + tileSide + rowSpacing * 2 +
padding * 2` (**176** — минимум, при котором видна хотя бы одна строка плиток).

`Config.Calendar`: `dayCellHeight = 28`, `todayCircleSide = 24`,
`rowSpacing = 4`, `sectionSpacing = 8`, `maxWeeks = 6` (31 день, начинающийся в
последний день недели, занимает ровно шесть строк), `headerHeight = 22`,
`weekdayRowHeight = 16`, `gridHeight = maxWeeks * dayCellHeight +
(maxWeeks - 1) * rowSpacing` (**188**).

`Config.Media`: `artworkCornerRadius = 6`, `rowSpacing = 12`, `textSpacing = 4`,
`positionLabelSpacing = 2`, `controlSpacing = 8` — раньше стояли числами прямо
в `PlayerView`.

`AdapterPaths`: `scriptName = "mediaremote-adapter.pl"`,
`frameworkName = "MediaRemoteAdapter.framework"`, `buildDirectoryName = ".build"`
(по нему опознаётся запуск из дерева сборки: у установленного приложения такого
каталога в пути нет), `vendorScriptPath`, `vendorFrameworkPath`.

`AppPaths.weather` — `weather.json` рядом с историей и настройками.

---

### Константы, добавленные починками 22.08

Дублируются здесь, потому что раздел `Config` в задания агентам не попадает.

`Config.History`: **`maxTextBytes = 2 МБ`** — свой предел для текста, строже
картиночного, потому что текст лежит прямо в `index.json`, а тот перезаписывается на
каждое изменение. **`orphanBlobGrace = 60 секунд`** — насколько свежий файл уборка
осиротевших не трогает (иначе она снесла бы файл, который просто не успел попасть в
индекс). **`brokenIndexDateFormat = "yyyy-MM-dd-HHmmss"`** и
**`brokenIndexSuffix = "broken"`** — имя отложенного испорченного индекса.

`Config.Hotkey`: сочетание по умолчанию **⌃⌥V** (не ⌘⇧V — это системная «вставка без
форматирования»), `requiredModifiers` — хотя бы один из ⌘/⌃/⌥ обязателен, иначе
сочетание отобрало бы обычную букву во всех приложениях, `keyCodeRange = 0...127`,
`signature` — четырёхбуквенный код владельца регистрации в Carbon.

`Config.SettingsWindow` и `Config.Limits` — размеры окна настроек и допустимые
пределы каждого поля; полный список в отчёте задачи 26 и в самом файле.

Ещё: `Config.secondsInMinute`, `Config.bytesInMegabyte` (настройки крутят минуты и
мегабайты, а таймеры живут в секундах и байтах — перевод должен называться, а не
стоять числом), `Config.Weather.cityName`, `Config.Calendar.manualFirstWeekday`,
`Config.Player.showsArtwork/showsPositionBar`, `Config.Behavior.opensOnHover`.

---

### Task 31: Оконная подсистема — по находкам ревью 21.08

Ревью оконной подсистемы нашло три критичных дефекта и семь важных. Все они
сходятся в одну точку: **представление состояния окном живёт в приватном `switch`
внутри делегата, и его никто не может проверить тестом.** Поэтому задача не
«починить десять мест», а вынести представление в проверяемый слой и по дороге
закрыть находки.

Идёт **после** слияния задач 22, 23, 26 — все они правят `AppDelegate`, и делать
это одновременно нельзя.

**Files:**
- Create: `Sources/ChelkaCore/Notch/PanelFrames.swift` — чистые функции рамок
- Create: `Sources/ChelkaCore/Notch/PanelPresenter.swift` — представление состояния окном
- Create: `Sources/ChelkaCore/Notch/ScreenWatcher.swift` — пересчёт геометрии
- Modify: `Sources/ChelkaCore/App/AppDelegate.swift` — оставить жизненный цикл и действия меню
- Modify: `Sources/ChelkaCore/Notch/TriggerZone.swift`
- Test: `Tests/ChelkaCoreTests/PanelFramesTests.swift`
- Test: `Tests/ChelkaCoreTests/PanelPresenterTests.swift`

> ⚠️ **Учесть при починке: задача 22 изменила устройство обоих окон.** Приём файлов
> сделан не слоем поверх, а **корневой вью** — и в окне зоны над челкой (внутри неё
> живёт следящая вью с наведением и кликом), и в окне панели (вокруг содержимого).
> Слой поверх съел бы нажатия у плиток истории и убил бы перетаскивание наружу.
> Разведение приёма между зоной и панелью держится на том же правиле «зона молчит
> при раскрытой панели»: окно, игнорирующее мышь, прозрачно и для перетаскивания.
> Значит любое изменение порядка окон и интерактивности зоны надо проверять **и на
> приёме файлов тоже**, иначе полка молча перестанет принимать.

#### Критично 1: панель встаёт поверх зоны-триггера, и клик по челке умирает

Зона поднимается на передний план один раз при запуске, панель — на каждом
переходе, оба окна на уровне строки состояния. В состоянии наведения рамка панели
равна ровно вырезу, то есть панель полностью накрывает зону, оставаясь
интерактивной: `setInteractive(false)` стоит только для раскрытого состояния.

Наведение всегда происходит раньше клика. Значит нажатие попадает в панель, у
которой нет ни одного обработчика мыши, и съедается — **раскрытие недостижимо, то
есть главный жест приложения не работает никогда.** Второй сценарий из того же
корня: если система отдаёт «мышь ушла» для перекрытой области отслеживания,
получается цикл наведение — скрытие — наведение со скоростью цикла событий, то
есть мигание.

Почему не поймали: поднятие панели на передний план добавлено уже **после**
единственной живой трассировки, где клик, по признанию отчёта, «не проверен, а
выведен».

Починка: держать зону строго выше панели (свой уровень окна либо переупорядочивание
после каждого поднятия), либо перенести наведение и клик в саму панель, оставив
зону только для скрытого состояния.

#### Критично 2: карточка затирает содержимое раскрытой панели и не возвращает его

Запись содержимого возможна в любом состоянии, а восстановление — только когда
состояние равно «карточка». Сценарий: карточка выехала → в эти три секунды человек
выбирает в меню «Показать панель» → ставится раскрытое состояние и сетка истории →
через четверть секунды тикает таймер, очередь ещё не пуста → содержимое панели
заменяется чёрной фигурой карточки, растянутой на весь размер, сетка с поиском и
выделением уничтожена → очередь пустеет, но состояние раскрытое, и содержимое **не
восстанавливается никогда**.

Выхода из этого нет: `Esc` жил в сетке и умер вместе с ней, клик по челке в
раскрытом состоянии отключён, меню при раскрытой панели прячет «Показать панель» и
не предлагает «Скрыть». Остаётся выключить приложение. Обе жалобы владельца
возвращаются разом: содержимое устарело, и на экране висит карточка, держащая
клавиатуру.

Тест `suppressedWhilePanelExpanded` даёт ложное спокойствие: он доказывает, что
**новая** карточка не примется при раскрытой панели, но не то, что уже летящая
перестанет рисоваться.

Починка: рисование карточки — функция пары (состояние, очередь). Ничего не рисовать
вне состояния «карточка» и гасить очередь при выходе из него.

#### Критично 3: геометрия считается один раз, смена экрана не отслеживается

Спека требует прямым текстом: «пересчитывается на смену экрана и разрешения»,
«приложение не должно молча перестать работать при подключении монитора». Не
реализовано, наблюдателей нет, у зоны-триггера вообще нет способа переехать.

Сценарии: сменил масштаб в мониторах — панель и невидимая зона остались на старых
координатах, наведение не работает, а окно, съедающее клики, стоит где-то в строке
меню; вход с закрытой крышкой на док-станции — геометрия запасная, открыл крышку,
ничего не пересчиталось; два экрана — панель осталась на том, который был главным в
момент запуска.

Починка: подписаться на уведомление о смене параметров экранов, пересчитать
геометрию, переставить панель и зону, повторно применить текущее состояние.

#### Важно 4: наведение и раскрытие обходят расчёт рамки — то же расхождение, что чинили

Состояния наведения и раскрытия зовут изменение размера с плоским числом, а
карточка — расчётную функцию. Два разных пути, гарантии крыльев и минимальной
высоты есть только у одного.

Следствия: наведение получает размером ровно вырез — то есть область без пикселей,
и телу достаётся высота ноль: **состояние наведения не показывает ничего**, хотя
спека обещает тонкую подсказку. Крылья тоже отсутствуют. А раскрытая панель
получает 640×340 как **полную** высоту, из которой вырез съедает 38 — следующий,
кто подгонит сетку под 340, воспроизведёт ровно то обрезание, которое уже чинили.

Починка: одна чистая функция рамки от пары (состояние, геометрия), выраженная через
«размер содержимого плюс высота челки», с крыльями и минимальной высотой для всех
четырёх состояний; `expandedSize` сделать размером **содержимого**; тест на все
четыре состояния.

#### Важно 5: чёрную фигуру рисует только карточка, раскрытая панель делает челку заметнее

Раскрытая панель прижата к верху экрана, её верхние 38 точек залиты стеклом слева и
справа от выреза, верхние углы скруглены на границе экрана — челка читается как
чёрный укус в стеклянной панели. Это дословно первая жалоба владельца, вернувшаяся в
раскрытом состоянии.

Починка: любое состояние, прижатое к верху экрана, обязано рисовать полосу высотой
челки той же чёрной фигурой; стекло — только содержимому ниже.

#### Важно 6: карточка пересобирает всё дерево вью и перечитывает картинку четыре раза в секунду

Таймер тикает каждые четверть секунды и, пока очередь не пуста, безусловно создаёт
новое дерево вью, а чтение картинки сидит внутри отрисовки. За три секунды жизни
карточки со снимком (до 40 МБ) дерево пересоздаётся 12 раз, картинка читается с
диска и декодируется 12 раз, анимация появления начинается заново на каждом тике.

Починка: пересобирать содержимое только при смене события; чтение картинки вынести
из отрисовки.

#### Важно 7: из раскрытой панели нет выхода кликом

Отключение зоны в раскрытом состоянии убирает единственный источник клика в
приложении. Переход «раскрыто → скрыто» в рабочем приложении невозможен, а тест
утверждает переход, который нечем вызвать: тест зелёный, поведения нет.

Починка: дать раскрытому состоянию источник клика, не совпадающий с зоной по челке,
и добавить в меню пункт скрытия. ⚠️ Глобальный хоткей (задача 23) это частично
закрывает — повторное нажатие складывает панель, — но клик по челке всё равно обещан
спекой.

#### Важно 8: при отсутствии главного экрана приложение теряет даже аварийный выход

Ранний выход бросает весь запуск: ни панели, ни зоны, ни иконки в строке меню, ни
наблюдателя буфера, повторной попытки нет. Сценарий: автозапуск при входе, когда
дисплей ещё просыпается, — процесс живёт без единого элемента интерфейса, выключить
его можно только через мониторинг системы. Это прямо противоречит гарантии «выход
есть всегда», которую утверждает тест меню.

Починка: иконку в строке меню создавать первой и безусловно; геометрию — с падением
на первый доступный экран и повтором по уведомлению из критичного 3.

#### Важно 9: на запуске панель показана, хотя состояние скрытое

Окно поднимается на передний план при скрытом состоянии, а применение состояния на
запуске не зовётся ни разу. На машине без выреза после запуска в центре строки меню
висит видимая плашка, и она не исчезнет, пока не произойдёт первая пара
«вошёл-вышел» мышью. На челочной машине то же окно просто не видно — лежит в
вырезе, поэтому дефект и не заметили.

#### Важно 10: зона-триггер съедает строку меню на экранах без челки

Запасная геометрия даёт плашку по центру верхнего края, окно уровнем выше строки
меню и принимает мышь. На внешнем мониторе клик по заголовку меню открывает панель
4elka вместо меню. Плюс переключение идёт по нажатию и не передаёт событие дальше,
поэтому начатое в этой полосе перетаскивание не стартует вообще.

Починка: в запасной геометрии не накрывать строку меню, переключать по отпусканию
кнопки, необработанные события пропускать дальше.

#### Мелочи из того же ревью

- Тест высоты карточки перепечатывает литералы из вью (34 и 8), поэтому её не
  защищает: поднимешь миниатюру до 44 — тест зелёный, а содержимое снова обрезано.
  И `cardBodyHeight = 50` стоит ровно на вычисленном минимуме, без запаса.
- `ActivityCardView` держит десяток магических чисел против правила о константах.
- `resignKey()` не делает того, что обещает комментарий: рассылает уведомление, но
  не передаёт клавиатурный статус другому окну. Сейчас замаскировано тем, что выход
  из раскрытого состояния всегда убирает окно.
- `ConfigTests` сверяет константы с их же литералами — упасть может только если
  кто-то поменял `Config`, и починкой будет правка теста. Заменить на отношения
  (`imageLimit < textLimit`, `hysteresis < lowThreshold`) либо удалить.
- Заглушка настроек показывает модальное окно из приложения, которое никогда не
  активируется: окно может встать за чужими окнами и заблокировать главный поток,
  и тогда даже меню не открыть. Задача 26 эту заглушку убирает — проверить, что
  настоящее окно настроек активирует приложение перед показом.
- `items(panel:launchesAtLogin:)` не использует второй параметр, но три теста его
  передают — мёртвый параметр.
- Голая `+ 1` в расчёте высоты против правила о константах, а тест «фигура не ниже
  челки» проверяет только строгое больше, поэтому тело в одну точку его устроит.

- [ ] **Step 1: Падающие тесты на рамки всех четырёх состояний**
- [ ] **Step 2: Чистые функции рамок, `expandedSize` как размер содержимого**
- [ ] **Step 3: Представление состояния окном вынести из делегата и накрыть тестами**
- [ ] **Step 4: Порядок окон: зона строго выше панели, либо клик в самой панели**
- [ ] **Step 5: Рисование карточки — функция от пары (состояние, очередь)**
- [ ] **Step 6: Наблюдатель смены экранов**
- [ ] **Step 7: Запуск: иконка в строке меню первой, применение состояния до показа**
- [ ] **Step 8: Запасная геометрия не накрывает строку меню**
- [ ] **Step 9: Мелочи и тесты-пустышки**
- [ ] **Step 10: Коммит**

---

### Task 32: Настройки должны доходить до дела

**Зачем эта задача появилась (24.08).** Ревью нашло, что настройка сочетания клавиш
не применяется: выбор сохраняется на диск, а регистрируется всегда значение по
умолчанию. Проверка шире показала, что **это не единичный случай, а свойство всего
окна настроек**. Замерено грепом по `Sources`:

| Настройка | Применяется? |
|---|---|
| Квоты истории (текст, картинки, файлы), потолок размера | да |
| Список приложений, из которых не записывать | да |
| Время жизни карточки | да (`ActivityCenter`) |
| Открывать панель по наведению | да (`AppDelegate.hovered`) |
| Карточки от трека / от буфера / от заряда — три тумблера | **нет** |
| Показывать айфон | **нет** |
| Обложка, полоса позиции | **нет** |
| Пороги заряда и гистерезис | **нет** (`BatteryAlerts` берёт из `Config`) |
| Координаты, интервал обновления, порог устаревания погоды | **нет** (`requestURL` статическая) |
| Первый день недели | **нет** |
| Сочетание клавиш | **нет** (`GlobalHotkey.installed` зовётся без аргумента) |

То есть человек крутит ручки, значения честно пишутся в `settings.json` — и **ничего
не происходит**. Это хуже отсутствующей настройки: отсутствующая честна, а эта врёт.

**Правило, которое отсюда следует и уже записано в общую часть:** настройка,
добавленная в окно, обязана быть подключена той же задачей. Раздел, который нечем
подключить, в окно не добавляется.

Идёт **после задачи 30** (там подключаются сами подсистемы: пороги заряда, погода и
календарь без подключённых виджетов протягивать некуда).

**Files:**
- Modify: `Sources/ChelkaCore/App/AppDelegate.swift` — применение настроек при старте и на изменение
- Modify: `Sources/ChelkaCore/Activity/ActivityCenter.swift` — три тумблера источников
- Modify: `Sources/ChelkaCore/Hotkey/GlobalHotkey.swift` — причина отказа наружу
- Modify: `Sources/ChelkaCore/Core/HotkeyChoice.swift` — вычистить список сочетаний
- Modify: `Sources/ChelkaCore/Core/SettingsSanitize.swift` — гистерезис меньше расстояния между порогами
- Test: `Tests/ChelkaCoreTests/SettingsApplicationTests.swift`

- [ ] **Step 1: Падающий тест на то, что настройка доходит**

Ключевая мысль: проверять надо **не «значение сохранилось»**, а «поведение
изменилось». Тест на сохранение уже есть, и он ничего не поймал.

```swift
@Test func cardSourceTogglesActuallySilenceTheirSource() {
    // Выключенный источник не должен ставить карточку в очередь вовсе — иначе
    // тумблер в настройках только делает вид, что работает.
    var settings = Settings.defaults
    settings.cardsFromTrack = false
    let center = ActivityCenter(settings: { settings })
    center.submit(ActivityEvent(kind: .track, title: "трек", subtitle: nil))
    #expect(center.queue.current == nil)
}

@Test func everySettingIsReachableFromSomewhere() {
    // Страховка от повторения: список настроек, у которых есть применяющая
    // сторона. Добавил поле в Settings — либо подключи, либо не добавляй.
    #expect(SettingsApplication.unappliedFields.isEmpty)
}
```

Второй тест — это защита от возврата дефекта, и он важнее первого: он ловит
**класс** ошибки, а не один случай.

- [ ] **Step 2: Сочетание клавиш**

`GlobalHotkey.installed(combo:)` звать с сочетанием из настроек; в `settingsChanged`
снимать старую регистрацию и ставить новую. Отказ регистрации возвращать причиной,
а не только строчкой в журнал: перечисление с двумя случаями — «занято другим
приложением» (`eventHotKeyExistsErr`) и «этот экземпляр уже зарегистрирован» —
сейчас оба сообщаются одним текстом, то есть текст может врать.

Причину показывать в разделе «Поведение» рядом с выбором сочетания. Человек, у
которого сочетание занято Alfred или Raycast, сейчас нажимает и не понимает, почему
тихо, а узнать может только из `Console.app`.

- [ ] **Step 3: Вычистить список предлагаемых сочетаний**

`Config.Hotkey` на тридцать строк объясняет, почему нельзя брать ⌘⇧V (системная
«вставка без форматирования»), а список выбора при этом предлагает `⌃Space`
(переключение раскладки), `⌥Space` (неразрывный пробел), `⌘⇧C` (Finder «Компьютер»)
и `⌘⇧B` (Safari «Панель закладок»). Убрать. Тест — на **весь** список, а не на
сочетание по умолчанию: `HotkeyChoice.all` не должен содержать ничего из явного
списка занятых системой.

- [ ] **Step 4: Остальное протянуть**

Пороги заряда с гистерезисом — аргументом в `BatteryAlerts.evaluating`. Координаты,
интервал и порог устаревания — параметрами в `WeatherProvider` (и `requestURL`
сделать не статической). Первый день недели — в построение сетки месяца. Обложка и
полоса позиции — в `PlayerView`. Показ айфона — в поставщик зарядов. Значения из
`Config` остаются значениями по умолчанию.

- [ ] **Step 5: Санитайзер**

`sanitized()` гарантирует только «нижний порог меньше верхнего». Набор
`low = 99, high = 100, hysteresis = 1` проходит целиком, и батарея, гуляющая между
98 и 100, будет выдавать «заряд на исходе» на 98 процентах. Гистерезис обязан быть
меньше расстояния между порогами. Тест — на пользовательский ввод, а не на `Config`:
от константы это уже требуется, а от человека никто не требует.

- [ ] **Step 6: Коммит**

```bash
git add Sources Tests
git commit -m "fix: настройки применяются, а не только сохраняются"
```
