...
- **Координаты погоды:** широта 51.1605, долгота 71.4704 (Астана), таймзона `Asia/Almaty`.
...
- Produces:
  ...
  @MainActor public final class ClipboardCoordinator: ObservableObject {
      ...
      @Published public private(set) var activity: ActivityQueue
...
private func makeCoordinator(panel: @escaping () -> PanelState = { .hidden })
    -> (ClipboardCoordinator, BlobStore) {
    ...
    return (ClipboardCoordinator(capture: capture, index: index, blobs: blobs, panelState: panel), blobs)
}
...
@Test @MainActor func storesItemAndRaisesCard() {
    ...
    #expect(c.activity.current?.kind == .clipboard)
    #expect(c.activity.current?.title == "привет")
}
...
@Test @MainActor func ignoredCopyRaisesNoCard() {
    ...
    #expect(c.activity.current == nil)
}

@Test @MainActor func noCardWhilePanelExpanded() {
    let (c, _) = makeCoordinator(panel: { .expanded })
    c.handle(snap("привет"), now: Date())
    #expect(c.history.items.count == 1)
    #expect(c.activity.current == nil)
}
...
@Test @MainActor func cardDisappearsAfterDuration() {
    ...
    c.tick(now: t.addingTimeInterval(Config.Activity.duration + 0.1))
    #expect(c.activity.current == nil)
}
...
@Test @MainActor func historySurvivesRestart() {
    ...
    let first = ClipboardCoordinator(capture: capture, index: index, blobs: blobs, panelState: { .hidden })
    first.handle(snap("сохранись"), now: Date())

    let second = ClipboardCoordinator(capture: capture, index: index, blobs: blobs, panelState: { .hidden })
...
public final class ClipboardCoordinator: ObservableObject {
    ...
    public func handle(_ snapshot: PasteboardSnapshot, now: Date) {
        ...
        activity = activity.submitting(Self.activityEvent(for: item),
                                      now: now, panel: panelState())
        scheduleSave()
    }

    public func tick(now: Date) { activity = activity.ticking(now: now) }
...
В делегате: создать `BlobStore`/`HistoryIndex`/`ClipboardCapture`/`ClipboardCoordinator`, запустить `PasteboardWatcher` с `onChange` в координатор, повесить таймер раз в 0.25 с на `coordinator.tick(now:)`, и в состоянии `.activity` показывать `ActivityCardView` через `NSHostingView`. Панель в состоянии `.activity` должна иметь размер челки по ширине плюс запас — используй `CGSize(width: max(geometry.rect.width + 260, 320), height: 54)`.
...
Добавить тест на имена, потому что от них зависит, что упадёт в Finder:
...
Общая точка отправки карточек должна быть одна на всё приложение — её объявляем
в делегате (Task 10 создаёт очередь, здесь появляется метод отправки):
...
func submitActivity(_ event: ActivityEvent) {
    activityQueue = activityQueue.submitting(event, now: Date(), panel: machine.state)
    apply { $0.showingActivity() }
    renderActivity()
}
```

Этот же метод передаётся в `MediaCoordinator(submitActivity:)` из Task 15 и вызывается
координатором буфера. Подключение батарей:
...
public final class WeatherProvider: ObservableObject {
    ...
    public static var requestURL: URL {
        ...
        components.queryItems = [
            ...
            .init(name: "timezone", value: Config.Weather.timezone),
        ]
...
public enum Transformations {
    ...
    private static func timestamp(_ input: String) -> String? {
        ...
        formatter.timeZone = TimeZone(identifier: Config.Weather.timezone)
