...
- **Сборка обязана быть без предупреждений.** `swift build` на чистом дереве не должен печатать ни одного `warning:`. Работа с AppKit из неизолированного контекста даёт их десятками — типы, которые трогают окна и вью, помечаются `@MainActor`. Проверять только на чистой сборке: повторный `swift build` ничего не перекомпилирует и покажет обманчивый ноль.
