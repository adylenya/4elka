...
public final class NotchPanel: NSPanel, PanelSurface {
    ...
    /// уже полностью видно (прозрачность равна единице) и просто меняет
    /// размер (наведение → раскрытие), повторный цикл «спрятать и показать»
    /// заново дал бы лишнюю вспышку прозрачности.
    ///
    /// Решение по одной лишь видимости окна (`isVisible`), без проверки самой
    /// прозрачности, было ошибкой: окно СЧИТАЕТСЯ видимым и в момент, когда
    /// `hide()` только начал гасить его до нуля — оно ещё на экране, просто
    /// прозрачность падает. Быстрое наведение-и-снятие (обычное движение мыши
    /// мимо челки) запускало обе анимации почти одновременно; эта функция
    /// видела «уже видимо» и молча ничего не делала — а гаснущая анимация
    /// доводила прозрачность до нуля и по завершении убирала окно совсем.
    /// Панель считала себя раскрытой, а на экране её не было ни точки:
    /// «клик и наведение не работают вовсе».
    public func show() {
        if isVisible, alphaValue == 1 {
            orderFrontRegardless()
            return
        }
        // Не на экране вовсе — стартуем с невидимого, чтобы появление было
        // плавным. Если же окно уже на экране, но прозрачность ниже единицы
        // (гаснет по чужой команде), трогать её нельзя: новая анимация должна
        // подхватить её ровно с текущего значения, а не откатить назад к нулю.
        if !isVisible {
            alphaValue = 0
        }
        orderFrontRegardless()
