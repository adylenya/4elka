...
public enum Config {
    ...
    public enum Panel {
        ...
        /// Значок и градусы погоды — крупнее рядового текста строки: погода
        /// была едва заметной строкой того же размера, что и подписи устройств,
        /// хотя это единственное, что стоит увидеть с первого взгляда.
        public static let weatherIconSize: CGFloat = 26
        public static let weatherTemperatureSize: CGFloat = 20
        /// Зазор между строками списка устройств — свой, отдельный от
        /// `rowSpacing`: список тянулся вниз тем же зазором, что разделяет
        /// целые разделы полосы, и выглядел рыхлым и занимающим слишком много.
        public static let deviceRowSpacing: CGFloat = 3
    }
    ...
    public enum Weather {
        ...
        /// Сколько часов прогноза просить у Open-Meteo, включая текущий.
        /// Текущий час дублирует то, что уже показано отдельно, и отбрасывается
        /// при разборе — владельцу видно четыре часа вперёд, не больше: строка
        /// в нижней полосе панели не бесконечна.
        public static let forecastHours = 5
