...
public enum AtomicFile {
    ...
    public static func write(_ data: Data, to url: URL) throws {
        ...
        do {
            ...
            // Байты обязаны лежать на диске ДО подмены. Подмена атомарна только
            // относительно читателя: ядро вправе держать содержимое временного
            // файла в кэше и записать его после переименования, и тогда потеря
            // питания оставляет на месте индекса недописанный файл — ровно тот
            // сценарий, из-за которого полка однажды стиралась насовсем.
            try fsync(tmp)
    ...
    /// Сброс байтов файла на носитель. Отдельной функцией, потому что закрыть
    /// дескриптор надо и когда сброс не удался: иначе неудачная запись утекала
    /// бы открытым файлом на каждый вызов.
