...
public final class ServiceContainer {
    ...
    public func start() {
        ...
        isRunning = true
        media.start()
        devices.onUpdate = { [weak self] _, next in self?.handleDevices(next) }
        devices.start()
        weather.start()
    }
    ...
    func handleDevices(_ measured: [DeviceCharge]) {
        let (next, fired) = alerts.evaluating(visibleDevices(from: measured),
